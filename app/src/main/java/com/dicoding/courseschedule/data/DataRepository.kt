...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getNearestSchedule(queryType: QueryType): LiveData<Course?> {
        val nearestScheduleQuery = nearestQuery(queryType)
        return dao.getNearestSchedule(nearestScheduleQuery)
    }

    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> {
        val allCourseQuery = sortedQuery(sortType)
        val configuration = PagedList.Config.Builder().setPageSize(PAGE_SIZE).build()
        return LivePagedListBuilder(dao.getAll(allCourseQuery), configuration).build()
    }
